...
if ($tipoItem == "todos" || $tipoItem == "receitas") {
    ...
    $receitas = executarConsultaPreparada($db, $sqlReceitasTexto, $tiposReceitas, $valoresReceitas);
...
if ($tipoItem == "todos" || $tipoItem == "gastos") {
    ...
    $gastos = executarConsultaPreparada($db, $sqlGastosTexto, $tiposGastos, $valoresGastos);
...
<div class="container">
    <h2><?php echo htmlspecialchars($dados_controle["nome_controle"] ?? "Controle financeiro"); ?></h2>
    <p style="text-align:center; color:var(--texto-secundario);"><strong>Detalhes do controle financeiro</strong></p>

    <div class="resumo">
        <p><strong>Salário / renda principal:</strong> <?php echo Formatador::formatarMoeda($dados_controle["salario"], $dados_controle["moeda"] ?? "BRL"); ?></p>
        <p><strong>Total de rendas:</strong> <?php echo Formatador::formatarMoeda($dados_controle["total_rendas"], $dados_controle["moeda"] ?? "BRL"); ?></p>
        <p><strong>Total de gastos:</strong> <?php echo Formatador::formatarMoeda($dados_controle["total_gastos"], $dados_controle["moeda"] ?? "BRL"); ?></p>
        <p><strong>Sobra:</strong> <?php echo Formatador::formatarMoeda($dados_controle["sobra"], $dados_controle["moeda"] ?? "BRL"); ?></p>
        ...
        <?php if (!empty($dados_controle["nome_parceiro"]) || floatval($dados_controle["renda_parceiro"] ?? 0) > 0) { ?>
            <p><strong>Segunda pessoa:</strong> <?php echo htmlspecialchars($dados_controle["nome_parceiro"] ?: "Pessoa 2"); ?> — <?php echo Formatador::formatarMoeda($dados_controle["renda_parceiro"] ?? 0, $dados_controle["moeda"] ?? "BRL"); ?></p>
        <?php } ?>
        <p><strong>Moeda:</strong> <?php echo htmlspecialchars(Formatador::nomeMoeda($dados_controle["moeda"] ?? "BRL")); ?></p>
    ...
    <?php if ($buscaItens != "") { ?>
        <div class="resumo-busca">
            ...
            <?php if ($tipoItem == "todos" || $tipoItem == "receitas") { ?>
                Receitas encontradas: <?php echo count($receitasLista); ?> | Total: <?php echo Formatador::formatarMoeda($totalReceitasFiltradas, $dados_controle["moeda"] ?? "BRL"); ?><br>
            <?php } ?>
            <?php if ($tipoItem == "todos" || $tipoItem == "gastos") { ?>
                Gastos encontrados: <?php echo count($gastosLista); ?> | Total: <?php echo Formatador::formatarMoeda($totalGastosFiltrados, $dados_controle["moeda"] ?? "BRL"); ?>
            <?php } ?>
        </div>
    <?php } ?>
    ...
    <div class="area-botao">
        <a class="btn-adicionar" href="adicionar_receita_historico.php?id=<?php echo $id_controle; ?>">+ Adicionar receita</a>
    </div>
    <div class="tabela-scroll"><table>
        ...
        <?php foreach ($receitasLista as $linha) { ?>
            <tr>
                ...
                <td><?php echo Formatador::formatarMoeda($linha["valor"], $dados_controle["moeda"] ?? "BRL"); ?></td>
    ...
    <h3>Gastos</h3>
    <div class="area-botao">
        <a class="btn-adicionar" href="adicionar_gasto_historico.php?id=<?php echo $id_controle; ?>">+ Adicionar gasto</a>
    </div>
    <div class="tabela-scroll"><table>
        <tr>
            <th>Nome do gasto</th>
    ...
    <div class="links">
        <a href="exportar_csv.php?id=<?php echo $id_controle; ?>" style="background: var(--verde, #16a34a); color: white; padding: 10px 15px; border-radius: 8px; text-decoration: none; font-weight: bold; display: block; width: fit-content; margin: 0 auto 15px auto;">📥 Baixar Relatório (Excel)</a>
