...
if ($nome_controle === "") { $nome_controle = "Controle financeiro"; }
$moeda = moedaPermitida($_POST["moeda"] ?? "BRL");
$nome_parceiro = trim($_POST["nome_parceiro"] ?? "");
$renda_parceiro = floatval($_POST["renda_parceiro"] ?? 0);
if ($renda_parceiro < 0) { $renda_parceiro = 0; }
...
(id_usuario, usuario, nome_controle, nome_parceiro, salario, renda_parceiro, moeda, total_rendas, total_gastos, sobra, porcentagem_gasta, situacao)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<div class="container">
    ...
    <p><strong>Salário / renda principal:</strong> <?php echo formatarMoeda($salario, $moeda); ?></p>
    <?php if ($renda_parceiro > 0 || $nome_parceiro !== "") { ?><p><strong>Segunda pessoa:</strong> <?php echo htmlspecialchars($nome_parceiro !== "" ? $nome_parceiro : "Pessoa 2"); ?> — <?php echo formatarMoeda($renda_parceiro, $moeda); ?></p><?php } ?>
    <p><strong>Receitas adicionais:</strong> <?php echo formatarMoeda($total_receitas_adicionais, $moeda); ?></p>
    <p><strong>Total de rendas:</strong> <?php echo formatarMoeda($total_rendas, $moeda); ?></p>
    <p><strong>Total de gastos:</strong> <?php echo formatarMoeda($total_gastos, $moeda); ?></p>
    <p><strong>Sobra:</strong> <?php echo formatarMoeda($sobra, $moeda); ?></p>
    ...
    <div class="lista">
        ...
        <?php for ($i = 0; $i < count($nomes_gastos); $i++) { ?>
            <p>
                ...
                <?php echo formatarMoeda(floatval($valores_gastos[$i]), $moeda); ?>
                <br>
