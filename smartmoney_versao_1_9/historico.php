...
$tiposPermitidos = ["todos", "gastos", "receitas"];
...
if ($busca != "") {
    ...
    if ($tipoBusca == "gastos") {
        $whereHistorico .= " AND EXISTS (SELECT 1 FROM gastos g2 WHERE g2.id_controle = c.id_controle AND g2.tipo_gasto LIKE ?)";
        $tiposHistorico .= "s";
        $valoresHistorico[] = $termoBusca;
    } elseif ($tipoBusca == "receitas") {
        $whereHistorico .= " AND EXISTS (SELECT 1 FROM receitas r2 WHERE r2.id_controle = c.id_controle AND r2.tipo_receita LIKE ?)";
        $tiposHistorico .= "s";
        $valoresHistorico[] = $termoBusca;
    } else {
        $whereHistorico .= " AND (
            EXISTS (SELECT 1 FROM gastos g2 WHERE g2.id_controle = c.id_controle AND g2.tipo_gasto LIKE ?)
...
$resultado = executarConsultaPreparada($conn, $sqlHistorico, $tiposHistorico, $valoresHistorico);
...
if (count($registros) > 0) {
    $moedaGrafico = moedaPermitida($registros[0]['moeda'] ?? 'BRL');
}
...
if ($busca != "") {
    ...
    if ($tipoBusca == "todos" || $tipoBusca == "gastos") {
        ...
        $resumoGastos = executarConsultaPreparada($conn, $sqlResumoGastos, "is", [$id_usuario, $termoBusca])->fetch_assoc();
    ...
    if ($tipoBusca == "todos" || $tipoBusca == "receitas") {
        ...
        $resumoReceitas = executarConsultaPreparada($conn, $sqlResumoReceitas, "is", [$id_usuario, $termoBusca])->fetch_assoc();
...
for ($i = 0; $i < $totalPontos; $i++) {
    ...
    $circulos .= "<circle cx='$x' cy='$y' r='5' class='ponto'><title>" . htmlspecialchars(simboloMoeda($moedaGrafico)) . " $valorFormatado</title></circle>";
...
for ($i = 0; $i <= 4; $i++) {
    ...
    $linhasGrade .= "
        ...
        <text x='10' y='" . ($yLinha + 4) . "' class='label-y'>" . htmlspecialchars(simboloMoeda($moedaGrafico)) . " $valorLinhaFormatado</text>
    ";
}
?>
...
<div class="container">
    ...
    <?php if ($busca != "") { ?>
        <div class="resumo-busca">
            ...
            <?php if ($tipoBusca == "todos" || $tipoBusca == "gastos") { ?>
                Gastos encontrados: <?php echo $qtdGastosBusca; ?> | Total: <?php echo formatarMoeda($totalGastosBusca, $moedaGrafico); ?><br>
            <?php } ?>
            <?php if ($tipoBusca == "todos" || $tipoBusca == "receitas") { ?>
                Receitas encontradas: <?php echo $qtdReceitasBusca; ?> | Total: <?php echo formatarMoeda($totalReceitasBusca, $moedaGrafico); ?>
            <?php } ?>
        </div>
    <?php } ?>
    ...
    <div class="tabela-scroll"><table>
        ...
        <?php if (count($registros) == 0) { ?>
            <tr><td colspan="10">Nenhum registro encontrado para os filtros informados.</td></tr>
        <?php } ?>
        ...
        <?php foreach ($registros as $linha) { ?>
            <tr>
                ...
                <td><?php echo formatarMoeda($linha["salario"], $linha["moeda"] ?? "BRL"); ?></td>
                <td><?php echo formatarMoeda($linha["total_rendas"], $linha["moeda"] ?? "BRL"); ?></td>
                <td><?php echo formatarMoeda($linha["total_gastos"], $linha["moeda"] ?? "BRL"); ?></td>
                <td><?php echo formatarMoeda($linha["sobra"], $linha["moeda"] ?? "BRL"); ?></td>
