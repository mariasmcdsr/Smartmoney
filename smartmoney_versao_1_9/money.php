...
$ultimo_salario = null;
$ultima_moeda = "BRL";
$ultimo_nome_parceiro = "";
$ultima_renda_parceiro = 0;

$consulta_salario = $conn->prepare("SELECT salario, moeda, nome_parceiro, renda_parceiro FROM controle_financeiro WHERE id_usuario = ? ORDER BY id_controle DESC LIMIT 1");
...
if ($resultado_salario->num_rows > 0) {
    ...
    $ultima_moeda = moedaPermitida($dados_salario["moeda"] ?? "BRL");
    $ultimo_nome_parceiro = $dados_salario["nome_parceiro"] ?? "";
    $ultima_renda_parceiro = floatval($dados_salario["renda_parceiro"] ?? 0);
}
?>
    ...
    <form action="salario.php" method="post">
        ...
        <?php if ($ultimo_salario !== null) { ?>
            <button type="button" class="btn-salario" onclick="usarUltimoSalario('<?php echo number_format($ultimo_salario, 2, '.', ''); ?>')">
                Usar salário anterior: <?php echo formatarMoeda($ultimo_salario, $ultima_moeda); ?>
            </button>
        <?php } else { ?>
